...
def knn_pytorch(x: torch.Tensor, K: int = 4) -> torch.Tensor:
    # Compute pairwise squared distances
    dist_matrix = torch.cdist(x, x, p=2)  # Euclidean distances
    # Get top K distances
    distances, _ = torch.topk(dist_matrix, K, dim=-1, largest=False)
    return distances

def knn_large_scale(x: torch.Tensor, K: int = 4, tile_size: int = 1000) -> torch.Tensor:
    N = x.size(0)
    distances = torch.zeros((N, K), device=x.device)
    for i in range(0, N, tile_size):
        tile = x[i:i+tile_size]
        dist_matrix = torch.cdist(tile, x, p=2)
        dist_k, _ = torch.topk(dist_matrix, K, dim=-1, largest=False)
        distances[i:i+tile_size] = dist_k
    return distances
